...
export const registerCandidate = async(req, res, next) => {
    try {
        ...
        return res.status(409).json({ error: "Registration Unsuccessful" })
    }
};
...
export const socketConnect = async(req, res) => {
    try {
        ...
        return res.status(200).json({ success: true, message: "Event triggered successfully" });
    } catch (error) {
...
export const votingCenterLogin = async(req, res) => {
    try {
        ...
        if (!ID || !password) {
            return res.status(400).json({ status: "failed", message: "ID and password are required" });
        }
        ...
        if (!votingCenter) {
            return res.status(404).json({ status: "failed", message: "Invalid ID or Password" });
        }

        // Check if user verified
        if (!votingCenter.is_verified) {
            return res.status(401).json({ status: "failed", message: "Voting center is not verified" });
        }

        // Compare passwords / Check Password
        if (votingCenter.password !== password) {
            return res.status(401).json({ status: "failed", message: "Invalid ID or password" });
        }
        ...
        res.status(200).json({
            voting_center: { ID: votingCenter.ID, email: votingCenter.email, name: votingCenter.name, role: votingCenter.role },
            status: "success",
...
export const admin_VC_Logout = async(req, res) => {
    try {
        ...
        await AdminRefreshToken.findOneAndUpdate({ token: refreshToken }, { $set: { blacklisted: true } });
...
export const getAllCandidatedetailsFromServer = async(req, res) => {
    try {
        ...
        if (candidates.length === 0) {
            return res.status(200).json({ success: true, message: "No candidate Available" });
        }
